/**
 * Nagarsevak AI – Unified Municipal Maintenance Scheduler
 * Advanced Decision Engine (Categories 1-20)
 * 
 * Updated with granular Category 10-20 rulesets.
 */
...
const ADVANCED_RISK_RULES = [
    ...
    // Note: Affects confidence primarily
    {
        id: 'CAT_13',
    ...
    // (Handled via logic modifiers, but keeping keywords for explicit mentions in logs)
    {
        id: 'CAT_18',
...
export const calculatePriorityScore = (issue, sensors = []) => {
    let score = 0;
    const factors = [];
    let explanations = [];
    ...
    const severityMap = { 'high': 30, 'medium': 20, 'low': 10 };
    ...
    const text = (issue.description + ' ' + (issue.title || '') + ' ' + (issue.metadata?.device?.userAgent || '')).toLowerCase();
    ...
    ADVANCED_RISK_RULES.forEach(rule => {
        if (rule.keywords.some(k => text.includes(k))) {
            ...
            if (rule.action === 'downgrade_confidence') {
                ...
                factors.push({ name: rule.name, value: -99, type: 'warning' }); // Visual flag only, score impact separate
    ...
    // If confidence drops below threshold OR explicit Fake detection
    if ((issue.aiAnalysis && !issue.aiAnalysis.isReal) || aiConfidence < 0.4) {
        ...
        escalationTarget = 'Vigilance Cell'; // Route fakes to vigilance
    ...
    let color = 'var(--color-emerald-500)';
    if (score >= 85) {
        riskLevel = 'Crisis';
    ...
    if (explanations.length > 0) {
        ...
        if (score >= 60) {
            // Limit to 2 important reasons for summary
    ...
    return {
        score,
        label: riskLevel, // Mapping Risk Level to Label for UI compat
        color,
        breakdown: factors,
        advancedAnalysis: {
            riskLevel,
